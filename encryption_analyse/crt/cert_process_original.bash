...
case $choice in
    1)
        ...
        if [ -f "${CERT_PATH}cert.pem" ]; then
            # 执行Python脚本
            python3 "$PYTHON_CA" -p "${CERT_PATH}cert.pem" --use-original-issuer -s "${CERT_PATH}cert_new.pem"
        else
            echo "can't cert.pem"
            exit 1
        fi
    ...
    2)
        ...
        if [ -f "${CERT_PATH}cert.pem" ]; then
            # 执行Python脚本
            python3 "$PYTHON_CUSTOM" -p "${CERT_PATH}cert.pem" --use-original-issuer -s "${CERT_PATH}cert_new.pem"
        else
            echo "can't cert.pem"
            exit 1
        fi
